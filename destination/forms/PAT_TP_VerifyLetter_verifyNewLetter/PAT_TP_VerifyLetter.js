FormManager.forms.PAT_TP_VerifyLetter = function (formKey, angularModule) {

    if (!(this instanceof FormManager.forms.PAT_TP_VerifyLetter))
        return new FormManager.forms.PAT_TP_VerifyLetter(formKey);

    BaseFormModule.call(this, formKey, "PAT_TP_VerifyLetter", angularModule);

    var _that = this;

    this.getClassName_ = function () {
        // return "com.fanap.fanrp.pat.business.dto.LetterFlowDTO";
        return "com.fanap.midhco.pat.process.PAT_TP_VerifyLetter_1";
    }

    this.getTemplateURL = function () {
        return "js/plugins/PAT/forms/PAT_TP_VerifyLetter/PAT_TP_VerifyLetter.html";
    }
        ...
        _that.getAngularModule().register.controller(controllerName, function ($scope, $q, $rootScope, $state, $http, notificationService, attachmentService, $sce) {
            ...
            var serializeForm = function () {
                ...
                for(var i = 0; i < $scope.letterFlowDTOs_content.length; i++){
                    out.letterFlowDTOs.unshift({
                        "id": $scope.letterFlowDTOs_content[i].id,
                        "parentFlowId": $scope.letterFlowDTOs_content[i].parentFlowId,
                        "personName": $scope.letterFlowDTOs_content[i].personName,
                        "personType": $scope.letterFlowDTOs_content[i].personType,
                        "forActionFlow": $scope.letterFlowDTOs_content[i].forActionFlow,
                        "deadline": $scope.letterFlowDTOs_content[i].deadline,
                        "description": $scope.letterFlowDTOs_content[i].description,
                        "flowBody": $scope.letterFlowDTOs_content[i].flowBody,
                        "vrPerson": $scope.letterFlowDTOs_content[i].vrPerson,
                        "approvementResult": $scope.letterFlowDTOs_content[i].approvementResult,
                        "verifyDate": $scope.letterFlowDTOs_content[i].verifyDate,
                        "objectHashCode": $scope.letterFlowDTOs_content[i].objectHashCode
                    });
                }
            ...
            var restoreForm = function (formData) {
                ...
                var previousLetFlowDTO = formData[2];
                ...
                $scope.date_input_persian = new JDate(parseInt($scope.letter.date_input)).toLocaleString();
                ...
                sqlDescriptorArray.push(
                    {
                        ...
                        "params": [{ "personId": $scope.letter.signer ? $scope.letter.signer.vrPerson : '' }]
                    },
                    {
