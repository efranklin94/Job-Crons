FormManager.forms.PAT_TP_ViewNewLetter = function (formKey, angularModule) {

    if (!(this instanceof FormManager.forms.PAT_TP_ViewNewLetter))
        return new FormManager.forms.PAT_TP_ViewNewLetter(formKey);

    BaseFormModule.call(this, formKey, "PAT_TP_ViewNewLetter", angularModule);
    // bundledProcessName:"پیش نمایش نامه"

    var _that = this;

    this.getClassName_ = function () {
        return "com.fanap.midhco.pat.process.PAT_TP_ViewNewLetter_1";
    }

    this.getTemplateURL = function () {
        return "js/plugins/PAT/forms/PAT_TP_ViewNewLetter/PAT_TP_ViewNewLetter.html";
    }
    ...
    this.defineController = function () {
        ...
        _that.getAngularModule().register.controller(controllerName, function ($scope, $q, $rootScope, $state, $http, notificationService, attachmentService, $sce) {
            ...
            var serializeForm = function () {
                ...
                if($scope.receiverReferralSet.length){
                    out.letter.receiverReferralSet = [];
            ...
            var restoreForm = function (formData) {
                ...
                sqlDescriptorArray.push(
                    ...
                    },
                    {
                        "sqlKey": "METHOD_getPerson",
                        "columnNames": ["id", "value"],
                        "params": [{ "personId": $scope.letter.vrCreator || $scope.letter.fromPerson ? $scope.letter.fromPerson.vrPerson : null }]
                    }
                );
                ...
                _that.batchSqlLoad(sqlDescriptorArray, $scope).then(function (res) {
                    ...
                    $scope.PATLetterTypeArray = res[j++];
